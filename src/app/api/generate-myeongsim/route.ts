...
1) 지금은 인생의 봄/여름/가을/겨울 중 어느 계절인지 (비유적 진단)
...
1) **재물 그릇 크기와 성격 진단**: 사주상의 재성 기운(정재/편재)의 강약 및 용신 관계와 자미두수 재백궁의 길흉(록존, 화록의 배치 여부 등)을 매칭하여 나의 재물 그릇 크기(S/M/L/XL 등급)를 다차원적으로 진단하고 상세한 근거를 동서양 관점으로 제시해 주세요.
...
4) **재물 그릇을 2배로 넓히는 무의식 리추얼 3가지**: 오행 보완 및 자미두수 길성의 주파수를 활성화하는 행동 처방.
5) **돈에 대한 결핍/불안을 해제하는 ACT(수용전념치료) 가치 중심 마음 확언.`
    },
    wealth_method: {
...
- **갈등 조정 강령**:
  ...
  2. 만약 서로 다르게 진단하는 부분(예: 사주에서는 길한데 자미두수에서는 흉하거나, 반대의 경우)이 있다면, 이를 모순으로 취급해 뭉뚱그리지 말고, **"자평명리(사주) 관점에서는 [외부 환경 및 결과적 양상]이 ~이고, 자미두수 관점에서는 [내적 심리 및 사건의 돌발적 기류]가 ~이다"**처럼 양쪽 학문의 고유한 관점과 분석 각도를 명확히 분리하여 100% 솔직하게 제시하십시오.
...
1) **사주 × 자미두수 재물 아키텍처 크로스 분석**: 사주 상의 재성(정재/편재)의 강약 및 오행 균형과 자미두수 재백궁(財帛宮)·전택궁(田宅宮)의 주성(예: 무곡, 천부, 태음 등) 및 길성/살성 배치를 상호 대조하여 내담자의 타고난 재물 그릇 크기와 금전 관리적 성향을 입체 진단하세요. (충돌 지점 발견 시 위의 갈등 조정 강령을 적극 반영하세요.)
...
5) **CBT 기반의 풍요 왜곡 인지 행동 교정 확언문 및 ACT 리추얼 처방`
    }
  };
...
동양 사주명리(십성, 격국, 용신, 대운, 세운, 합충, 신살)와 자미두수(12궁 × 100+별)의 핵심 데이터를 완벽히 교차 대조(Cross-Reference)하여, 내담자의 성향 and 인생 흐름을 세계 최고 수준의 정확도로 입체 진단 및 처방하십시오.
