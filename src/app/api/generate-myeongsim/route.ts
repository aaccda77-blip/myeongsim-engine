...
function getFrameworkPromptForPage(pageId: string, sajuProfile: any): { title: string; framework: string; prompt: string } {
  ...
  const mappings: Record<string, { title: string; framework: string; prompt: string }> = {
    p5_8: {
      ...
      framework: "CBT 인지행동코칭",
      prompt: `내담자의 타고난 일간(${dm})에 대한 본질 분석과 심리적 필터, 빛과 그림자를 인지행동코칭(CBT) 관점에서 분석해주세요. 일간 기질의 치우침으로 인해 일상에서 발생하는 누수 스트레스를 분석하고 따뜻한 어조로 교정 방안을 써주세요.`
    },
    p9_12: {
      title: "[핵심 기질 2] 현대적 기질 메타포",
      framework: "CBT 인지행동코칭",
      prompt: `내담자의 기질 메타포(${p.dayMasterAnalogy || '은빛 다이아몬드'})에 대해 비즈니스적 가치와 오작동 시의 자기파괴 메커니즘을 CBT 기법으로 분석해주세요. 왜곡된 인지 오류를 해제하는 맞춤 확언을 제안해주세요.`
    },
    p13_16: {
    ...
    p17_20: {
      ...
      framework: "CBT 인지행동코칭",
      prompt: `내담자의 일주 기질을 바탕으로 이동, 이직, 독립 등 삶의 주요 결정을 내릴 때 주도권을 쥐는 공간적 확장 전략과 커리어 가이드를 인지행동코칭(CBT) 관점에서 설계해주세요.`
    },
    p21_24: {
      title: "[심화 분석 1] 과다 십신의 폭주 제어",
      framework: "MBCT 마음챙김 인지코칭",
      prompt: `내담자의 과잉 십신으로 인해 발생하는 과밀 행동(예: 완벽주의, 불안 등)의 신경망 루프를 마음챙김 인지코칭(MBCT) 기법으로 분석하고, 의식적 자각을 통해 뇌를 안정시키는 비즈니스 리추얼을 가이드해주세요.`
    },
    p25_28: {
    ...
    p29_32: {
      ...
      framework: "DBT 변증법적 행동코칭",
      prompt: `결핍된 기운으로 인해 고질적으로 겪는 소통의 한계를 변증법적 행동코칭(DBT)의 대인관계 효율성 기법으로 분석하고, 이를 대안적 시스템이나 말하기 코드로 채워나가는 메타 소통법을 설계해주세요.`
    },
    p33_36: {
      title: "[포커스 월간 운세 1] 기회의 달 폭발 전략",
      framework: "ACT 수용전념코칭",
    ...
    p37_40: {
      ...
      framework: "ACT 수용전념코칭",
      prompt: `주의해야 할 리스크 달에 발생 가능한 판단 착오와 갈등 요소를 예방하고, 감정의 폭풍 속에서 관찰자 자아(Self-as-Context)로 신속히 복귀하기 위한 위기 관리 SOP를 가이드해주세요.`
    },
    p41_46: {
      title: "[현재 대운 분석] 인생의 거대한 파도",
      framework: "MBSR 스트레스 완화",
      prompt: `현재 주도하는 10년 대운이 가해오는 압박을 스트레스 완화(MBSR) 관점에서 자아 탄력성으로 전환하는 명심 멘탈 바이오해킹 기술을 다루고, 신체적 긴장 완화 가이드를 가이드해주세요.`
    },
    p47_50: {
    ...
    p55_59: {
      ...
      framework: "MBCT 마음챙김 인지코칭",
      prompt: `내면 깊숙이 자리 잡은 고위험 핵심 신념과 방어기제의 뿌리를 해부하고, 칼날 같은 방어기제를 내려놓고 안전지대를 구축하는 MBCT 3분 마음챙김 호흡 프로토콜을 가이드해주세요.`
    },
    p60_64: {
    ...
    p81_84: {
      ...
      framework: "MSC 자기자비 마음챙김",
    prompt: `천간 합과 지지 기운을 통해 강렬하게 끌리는 이상형과 친밀감 속에서 발현되는 다크 뉴럴 애착 패턴을 분석하고, 욕구를 우아하게 소통하는 연애 코드를 가이드해주세요.`
    },
    p88_90: {
      title: "[관계 리스크] 검열의 함정",
      framework: "DBT 변증법적 행동코칭",
      prompt: `파트너를 무의식적으로 검열하고 지치게 만드는 갈등 루프를 분석하고, 상대의 불완전함을 타당화(Validation)하며 존엄을 지켜내는 DBT DEAR MAN 대화 기술을 가이드해주세요.`
    },
    p91_94: {
    ...
    p95_98: {
      title: "[오행 솔루션] 신경학적 개운 가이드",
      framework: "MBSR 스트레스 완화",
      prompt: `부족한 기운을 일상 공간 인테리어와 배치를 통해 조율하는 공간 에너지 아키텍처 및 뇌를 자극하는 개운 환경 가이드(색상, 숫자, 방향)을 제시해주세요.`
    },
    p99_102: {
    ...
    // ─── Part 5. 2026년 월별 운세 마스터 플랜 ───
    monthly_q1: {
      title: "[2026 1~3월] 봄 에너지 · 기회의 씨앗 뿌리기",
      framework: "ACT 수용전념코칭",
      prompt: `2026년 1월(병인월), 2월(정묘월), 3월(무진월)의 월간 에너지가 내담자의 일간(${dm})과 어떻게 상호작용하는지 각 달별로 매우 구체적으로 분석해주세요. 각 달마다:
1) 주의해야 할 날짜 또는 기간 (구체적 날짜 범위 제시)
2) 돈·일·관계에서 각각 주의할 점 (3가지씩 리스트)
3) 이 시기에 밀어붙여야 할 것과 정리해야 할 것
4) ACT(수용전념코칭) 기반 감정 수용 전략
각 달을 2단 구조로 나눠 '기회 영역'과 '리스크 영역'으로 명확히 구분해주세요.`
    },
    monthly_q2: {
      title: "[2026 4~6월] 여름 에너지 · 밀어붙일 황금 타이밍",
      framework: "CBT 인지행동코칭",
      prompt: `2026년 4월(기사월), 5월(경오월), 6월(신미월)의 월간 에너지가 내담자의 일간(${dm})과 어떻게 상호작용하는지 각 달별로 매우 구체적으로 분석해주세요. 각 달마다:
1) 주의해야 할 날짜 또는 기간 (구체적 날짜 범위 제시)
2) 돈·일·관계에서 각각 주의할 점 (3가지씩 리스트)
3) 공격적으로 밀어붙일 최적 타이밍과 브레이크를 밟아야 할 순간
4) CBT 기반 인지 왜곡 교정 확언
각 달의 핵심 키워드와 행운의 방향, 행운의 컬러, 행운의 숫자를 포함해주세요.`
    },
    monthly_q3: {
      title: "[2026 7~9월] 가을 에너지 · 수확과 리스크 방어",
      framework: "MBCT 마음챙김 인지코칭",
      prompt: `2026년 7월(임신월), 8월(계유월), 9월(갑술월)의 월간 에너지가 내담자의 일간(${dm})과 어떻게 상호작용하는지 각 달별로 매우 구체적으로 분석해주세요. 각 달마다:
1) 주의해야 할 날짜 또는 기간과 구체적 사건 예고
2) 돈·일·관계에서 각각 주의할 점 (3가지씩 리스트)
3) 수확해야 할 영역과 정리·방어해야 할 영역
4) MBCT 마음챙김 기반 3분 호흡 프로토콜과 스트레스 대응법
각 달의 위험도를 5단계(🟢🟡🟠🔴⚫)로 표시해주세요.`
    },
    monthly_q4: {
      title: "[2026 10~12월] 겨울 에너지 · 정리와 내면 충전",
      framework: "MBSR 스트레스 완화",
      prompt: `2026년 10월(을해월), 11월(병자월), 12월(정축월)의 월간 에너지가 내담자의 일간(${dm})과 어떻게 상호작용하는지 각 달별로 매우 구체적으로 분석해주세요. 각 달마다:
1) 주의해야 할 날짜 또는 기간과 구체적 건강/재정 리스크
2) 돈·일·관계에서 각각 주의할 점 (3가지씩 리스트)
3) 올해를 정리하고 다음 해를 준비하는 전략적 체크리스트
4) MBSR 기반 보디스캔 명상 가이드와 연말 이완 리추얼
2026년 전체를 돌아보며 다가올 2027년을 미리 준비하는 전략도 제시해주세요.`
    },

    // ─── Part 6. 연도별 흐름 & 삶의 전환점 ───
    yearly_flow: {
      title: "[연도별 세운] 해마다의 밀기·정리 리듬 분석",
      framework: "ACT 수용전념코칭",
      prompt: `내담자의 일간(${dm})을 기준으로 2024년부터 2034년까지 향후 10년간의 연도별 세운 흐름을 매우 구체적으로 분석해주세요. 
각 연도마다:
1) 그 해의 천간·지지가 내담자에게 미치는 핵심 영향 한 줄 요약
2) 밀어붙여야 할 시기(월 단위)와 정리해야 할 시기
3) 점수(10점 만점)보다 중요한 흐름의 순서와 리듬
4) 해당 연도의 핵심 키워드 3개
표 형태로 정리하되, 특히 주의해야 할 해는 ⚠️ 마크, 기회의 해는 🌟 마크를 붙여주세요.`
    },
    life_turning: {
      title: "[대운·세운 전환점] 운명이 바뀌는 결정적 시기",
      framework: "MBSR 스트레스 완화",
      prompt: `내담자의 사주 원국과 대운 흐름을 바탕으로, 인생에서 운이 극적으로 바뀌는 결정적 전환점들을 구체적으로 분석해주세요.
1) 과거에 이미 지나간 전환점과 그때 실제 어떤 변화가 일어났을지 추정
2) 현재 진행 중인 대운의 핵심 에너지와 남은 기간 활용법
3) 다가올 다음 대운 전환 시기와 준비해야 할 구체적 행동
4) 좋은 시점을 최대한 활용하는 5단계 전략적 프로토콜
5) 전환기에 오는 심리적 불안을 다스리는 MBSR 기반 안정화 기법`
    },
    current_life: {
      title: "[현 시점 인생 흐름] 지금 나의 인생은 어디쯤인가?",
      framework: "MSC 자기자비 마음챙김",
      prompt: `내담자의 현재 나이, 대운, 세운을 종합하여 "지금 이 사람의 인생은 어디쯤에 와 있는가?"를 진심 어린 따뜻한 어조로 풀어주세요.
1) 지금은 인생의 봄/여름/가을/겨울 중 어느 계절인지 (비유적 분석)
2) 현재 가장 강하게 작용하는 에너지와 그 에너지가 만드는 일상의 패턴
3) 지금 시기에 절대 서두르면 안 되는 것 3가지
4) 지금 시기에 반드시 해야 하는 것 3가지
5) 자기자비(MSC) 관점에서 지금의 나에게 보내는 따뜻한 위로의 편지`
    },

    // ─── Part 7. 직업운 & 금전·재물운 ───
    career_fit: {
      title: "[직업 적합성] 나와 맞는 직업 vs 피해야 할 직업",
      framework: "CBT 인지행동코칭",
      prompt: `내담자의 일간(${dm}), 십신 분포, 오행 균형을 바탕으로 직업 적합성을 매우 구체적으로 분석해주세요.
1) 나와 잘 맞는 직업군 TOP 5 (각각 이유 설명)
2) 절대 피해야 할 직업군 TOP 3 (각각 이유 설명, 왜 에너지가 소모되는지)
3) 직장 내에서 빛나는 역할 유형 (리더/참모/전문가/크리에이터 등)
4) 직장 스트레스 발생 시 CBT 기반 인지 재구조화 스크립트
5) 내가 모르는 숨은 직업 적성 발굴 (오행 분석 기반 서프라이즈 추천)`
    },
    career_biz: {
      title: "[사업·프리랜서] 독립 적합성과 일할 때 주의점",
      framework: "DBT 변증법적 행동코칭",
      prompt: `내담자의 사주 원국을 바탕으로 사업/프리랜서/독립 적합성을 분석해주세요.
1) 사업 적합도 점수 (10점 만점)와 그 근거
2) 사업 시 반드시 함께해야 할 파트너 기질 (보완 오행)
3) 사업 시 가장 위험한 시기와 자금 관리 주의점
4) 프리랜서로 일할 때의 장점과 단점 (기질별 맞춤 분석)
5) 일할 때 감정 폭발/번아웃을 방지하는 DBT TIPP 프로토콜`
    },
    wealth_vessel: {
      title: "[재물 그릇] 돈이 들어오는 시기와 그릇의 크기",
      framework: "ACT 수용전념코칭",
      prompt: `내담자의 사주 오행/재성 분포/격국/용신과 함께 자미두수의 재백궁(財帛宮), 전택궁(田宅宮), 명궁(命宮)에 배치된 14주성 및 100여 개 주요 성좌(록존, 화록, 화기, 양타, 겁공 등)를 완전히 계산하여 상호 교차 대조(Cross-Reference) 분석을 수행해 주세요.
1) **재물 그릇 크기와 성격 분석**: 사주상의 재성 기운(정재/편재)의 강약 및 용신 관계와 자미두수 재백궁의 길흉(록존, 화록의 배치 여부 등)을 매칭하여 나의 재물 그릇 크기(S/M/L/XL 등급)를 다차원적으로 분석하고 상세한 근거를 동서양 관점으로 제시해 주세요.
2) **재물운의 인생 최고점 타이밍 (언제)**: 사주의 대운(大運)·세운(歲運) 흐름과 자미두수의 10년 대한(大限) 및 1년 유년(流年)을 입체적으로 대조하여 인생에서 돈이 가장 폭발적으로 들어오는 최고점 시기(구체적인 대운/대한 나이대, 세운의 연도 범위, 그리고 특히 에너지가 집중되는 핵심 월)를 족집게처럼 명확히 짚어주세요.
3) **재물 누수 위험기와 방어 전략**: 사주상의 충합(합충), 신살(겁살, 대재앙설 등)과 자미두수 재백궁/명궁의 화기(化忌), 살성(경양, 타라, 화성, 영성, 지공, 지겁)이 겹치는 재물 파괴 구간을 도출하고, 이를 방어하기 위한 현실적 자산 관리 프로토콜을 제시해 주세요.
4) **재물 그릇을 2배로 넓히는 무의식 리추얼 3가지**: 오행 보완 및 자미두수 길성의 주파수를 활성화하는 행동 가이드.
5) **돈에 대한 결핍/불안을 해제하는 ACT(수용전념코칭) 가치 중심 마음 확언.`
    },
    wealth_method: {
      title: "[돈 버는 방식] 나에게 맞는 최적의 수익 전략",
      framework: "CBT 인지행동코칭",
      prompt: `내담자의 사주 십신 구조(식상생재, 재생관, 관인상생 등) 및 격국(격국론), 용신과 자미두수의 관록궁(官祿宮), 재백궁(財帛宮), 천이궁(遷移宮)의 주성 및 보조길성(좌보, 우필, 천괴, 천월, 문창, 문곡 등)을 교차 분석하여 가장 빠르고 확실하게 부를 일구는 방식을 설계해 주세요.
1) **수익 모델 적합성 교차 분석 (어떤 방식으로)**: 
   - 급여형(정재/관인상생) vs 사업/창업형(편재/식상생재/살파랑 격) vs 전문 라이선스/기술형(인성/식신/창곡) vs 투자/자산 증식형(편재/록존/화록) 중 가장 완벽하게 정렬되는 부의 획득 방식을 우선순위대로 점수(100점 만점)와 함께 도출해 주세요.
   - 사주명리에서 지시하는 방향과 자미두수 관록궁/재백궁의 지시 방향이 어떻게 상호보완되고 일치하는지 그 교차 매커니즘을 상세히 설명해 주세요.
2) **나를 성공으로 이끄는 비즈니스 코어 전략**: 직장인, 사업가, 프리랜서 등 각각의 위치에서 부를 창출하기 위해 취해야 할 핵심 행동 강령.
3) **수익 극대화를 위해 유리한 분야 vs 패가망신을 피하기 위해 절대 손대면 안 되는 분야**: 사주 조후/오행과 자미두수 성좌를 활용한 맞춤형 업종/투자 카테고리 추천 및 금지 품목 지정.
4) **돈을 흘려보내는 무의식적 인지 오류와 행동 패턴 분석**: CBT 기법을 통한 소비/투자 왜곡 교정.
5) **CBT 기반의 풍요 주파수 정렬 확언문.`
    },

    // ─── Part 8. 건강운 & 연애·가족운 ───
    health_risk: {
      title: "[건강운] 취약 부위와 주의해야 할 시기",
      framework: "MBSR 스트레스 완화",
      prompt: `내담자의 오행 균형(목${p.elements?.wood || 0} 화${p.elements?.fire || 0} 토${p.elements?.earth || 0} 금${p.elements?.metal || 0} 수${p.elements?.water || 0})을 바탕으로 건강운을 구체적으로 분석해주세요.
1) 오행 과다/결핍으로 인한 취약 장기와 건강 문제 (심혈관, 소화기, 호흡기, 비뇨기, 근골격계 등)
2) 건강을 특히 주의해야 할 나이와 시기 (구체적 연도 제시)
3) 체력과 에너지 관리를 위한 오행 맞춤 운동법 (어떤 운동이 좋고 어떤 운동이 안 맞는지)
4) 스트레스가 신체에 미치는 경로와 MBSR 보디스캔 가이드
5) 일상에서 실천할 수 있는 건강 개운 리추얼 (음식, 색상, 방향)`
    },
    love_flow: {
      title: "[연애운] 관계 흐름과 끌림의 타이밍",
      framework: "DBT 변증법적 행동코칭",
      prompt: `내담자의 사주 원국(특히 일지 배우자궁, 천간합, 지지합)을 바탕으로 연애운을 구체적으로 분석해주세요.
1) 연애 에너지가 가장 강해지는 시기 (연도/월 구체적 제시)
2) 무의식적으로 끌리는 이상형의 기질 패턴 (오행/십신 기반)
3) 연인 관계에서 반복되는 갈등 루프와 그 무의식적 원인
4) 관계 속 거리 조절 기술 (너무 가까울 때 vs 너무 멀어질 때)
5) DBT DEAR MAN 기법을 활용한 파트너와의 건강한 소통 스크립트`
    },
    family_recovery: {
      title: "[가족운] 가까운 사람과의 거리 조절과 회복",
      framework: "MSC 자기자비 마음챙김",
      prompt: `내담자의 사주 원국(년주=부모, 월주=형제, 일주=배우자, 시주=자녀)을 바탕으로 가족운을 분석해주세요.
1) 부모와의 관계 패턴과 미해결 과제 (년주 분석)
2) 형제·동료와의 에너지 흐름 (월주 분석)
3) 배우자/파트너와의 깊은 무의식적 연결과 갈등 포인트 (일주 분석)
4) 자녀/후배와의 관계에서 주의할 점 (시주 분석)
5) 사랑과 가족 안에서의 회복 포인트 3가지 (MSC 자기자비 기반)`
    },
    life_summary: {
      title: "[인생 종합 브리핑] 운명의 전체 지도 한눈에 보기",
      framework: "MBCT 마음챙김 인지코칭",
      prompt: `내담자의 사주 원국, 십성, 격국, 용신, 대운/세운 흐름과 자미두수 명반 12궁 및 100+성좌, 대한/유년 흐름을 총망라하여 두 명리학적 도구가 가리키는 운명의 공통 궤적을 교차 대조(Cross-Reference) 방식으로 완벽하게 증명하고 풀어내십시오.
1) **인생의 계절과 시기 매핑**: 사주 대운의 조후/오행 흐름과 자미두수 대한의 궁위 길흉을 연결하여 인생의 봄(성장기), 여름(도약기), 가을(수확기), 겨울(성찰기)의 연도 범위와 핵심 미션을 규정해 주세요.
2) **인생 최고 황금기 TOP 3 (시기와 획득 방식)**: 사주 용신운/합국 및 자미두수 삼길화(화록, 화권, 화과)/록존 첩봉 등이 겹쳐 일어나는 인생 최고의 황금 시기(정확한 나이 및 년도)와 그때 잡아야 할 기회의 형태를 매우 구체적으로 짚어주세요.
3) **인생 취약 위기 구간 TOP 3 (시기와 예방법)**: 충, 형, 해, 파, 신살(백호, 괴강, 귀문 등) 및 자미두수 살성/화기가 재백궁, 질액궁, 부처궁 등을 치는 위기 시점(정확한 나이 및 년도)과 이를 메타 인지적으로 안전하게 대비할 프로토콜을 명시해 주세요.
4) **두 운명 학학이 가리키는 영혼의 사명 3가지**: 사주 용신이 지향하는 삶의 방향과 자미두수 명궁/복덕궁이 지시하는 영적 진화를 교차 해석.
5) **MBCT 3분 마음챙김 호흡 가이드 및 운명 관조 리추얼과 감동적인 마스터의 축복 축사.`
    },
    saju_ziwei_cross: {
      title: "[사주 × 자미두수] 초정밀 재물 최고점 및 부의 획득 전략",
      framework: "CBT · ACT 통합 명리치유 아키텍처",
      prompt: `내담자의 사주 원국(십성, 격국, 용신, 대운, 세운, 합충, 신살)과 자미두수 명반(12궁, 14주성, 록존, 화록, 화기, 살성 등 100여 개 별들의 배치)을 완전히 대조하여, 내담자의 인생 전체에서 '재물운의 최고점 타이밍'과 '가장 적합한 부의 획득 수단/방식'을 세계 최고 수준의 정확도로 도출해 주세요.

특히 사주와 자미두수의 데이터 해석상 상충이나 충돌이 발생할 경우, 아래의 **[⚖️ 명리 융합 데이터 갈등 조정 강령]**에 따라 고도화된 입체 해법을 제시하십시오:
- **갈등 조정 강령**:
  1. 두 체계가 가리키는 일치점(예: 사주 용신운과 자미두수 길성 첩봉이 겹치는 시기)을 최우선으로 확정하십시오.
  2. 만약 서로 다르게 분석하는 부분(예: 사주에서는 길한데 자미두수에서는 흉하거나, 반대의 경우)이 있다면, 이를 모순으로 취급해 뭉뚱그리지 말고, **"자평명리(사주) 관점에서는 [외부 환경 및 결과적 양상]이 ~이고, 자미두수 관점에서는 [내적 심리 및 사건의 돌발적 기류]가 ~이다"**처럼 양쪽 학문의 고유한 관점과 분석 각도를 명확히 분리하여 100% 솔직하게 제시하십시오.
  3. 이를 통해 단순한 길흉 충돌이 아닌, 내담자에게 "외부적으로는 성장하지만 내적으로는 단속을 해야 하는 입체적 통합 솔루션"이라는 더 넓고 유용한 통찰(Synthesis)을 전달하도록 서술하십시오.

다음 5가지 필수 요소를 매우 구체적으로 작성하십시오:
1) **사주 × 자미두수 재물 아키텍처 크로스 분석**: 사주 상의 재성(정재/편재)의 강약 및 오행 균형과 자미두수 재백궁(財帛宮)·전택궁(田宅宮)의 주성(예: 무곡, 천부, 태음 등) 및 길성/살성 배치를 상호 대조하여 내담자의 타고난 재물 그릇 크기와 금전 관리적 성향을 입체 분석하세요. (충돌 지점 발견 시 위의 갈등 조정 강령을 적극 반영하세요.)
2) **나에게 맞는 최적의 부의 획득 수단 (어떤 방식으로)**: 급여 소득형, 사업/창업/프랜차이즈형, 전문 기술/라이선스형, 금융/부동산 투자형 중 가장 정렬도가 높은 방식들을 우선순위 점수(100점 만점)와 함께 짚어주세요.
3) **인생 재물운의 최고점 골든 타이밍 (언제)**: 향후 10년 및 인생 전체를 통틀어 재물운이 가장 정점에 달하는 시기를 '구체적인 대운/대한 나이대', '세운(歲運)의 구체적인 연도 범위', 그리고 일년 중 '에너지가 극대화되는 구체적인 달(月)'까지 정확하게 짚어주세요. (사주 대운/세운과 자미두수 대한/유년이 다르게 보일 경우 양쪽 시각을 모두 쪼개서 적시하고 이를 유기적으로 봉합하세요.)
4) **재정적 붕괴를 예방할 리스크 방어막 (언제, 어떻게)**: 사주의 형충파해/신살 위험기와 자미두수의 화기(化忌) 및 살성(경양, 타라, 화성, 영성, 지공, 지겁)이 겹치는 자산 누수 시점을 경고하고, 구체적인 리스크 관리법을 제안하세요.
5) **CBT 기반의 풍요 왜곡 인지 행동 교정 확언문 및 ACT 리추얼 가이드`
    }
  };
...
// 💰 서버 사이드 레이트 리밋 (메모리 기반, Vercel Serverless 인스턴스별)
const rateLimitMap = new Map<string, { count: number; resetAt: number }>();
const RATE_LIMIT_WINDOW_MS = 60_000; // 1분
const RATE_LIMIT_MAX = 3; // 분당 최대 3회 Gemini API 호출
const FORCE_COOLDOWN_MS = 5 * 60_000; // force=true일 때 최소 5분 캐시 보호
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userRate) {
      if (now < userRate.resetAt) {
        if (userRate.count >= RATE_LIMIT_MAX) {
          console.warn(`🚫 [Rate Limit] user_id: ${userId} - 분당 ${RATE_LIMIT_MAX}회 초과`);
          return NextResponse.json({
            success: false,
            error: `API 보호: 1분에 최대 ${RATE_LIMIT_MAX}회만 생성할 수 있습니다. 잠시 후 다시 시도해 주세요.`
          }, { status: 429 });
        }
        userRate.count++;
      } else {
    ...
    // force=true여도 5분 이내 생성된 캐시는 재사용 (API 요금 보호)
    const { data: cacheData, error: cacheError } = await supabase
      .from('report_contents')
      .select('generated_text, updated_at')
      .eq('user_id', userId)
      .eq('page_id', pageId)
      .maybeSingle();
    ...
    if (cacheData && !cacheError) {
      ...
      // force=true이지만 5분 이내 생성된 캐시는 보호
      const updatedAt = new Date(cacheData.updated_at).getTime();
      if (now - updatedAt < FORCE_COOLDOWN_MS) {
        console.log(`🛡️ [Force Cache Protection] 5분 이내 캐시 반환 - user_id: ${userId}, page_id: ${pageId}`);
        return NextResponse.json({ success: true, text: cacheData.generated_text });
      }
    }
    ...
    // 자미두수 분석을 위한 메타데이터 추출 및 정규화
    const birthDate = finalProfile.birthDate || '미정(사주 원국 기준)';
    const birthTime = finalProfile.birthTime || '12:00';
    const calendarType = finalProfile.calendarType === 'solar' ? '양력(太陽曆)' : '음력(太陰曆)';
    const gender = finalProfile.gender === 'male' || finalProfile.gender === '남성' ? '남성(乾命)' : '여성(坤命)';
    
    const prompt = `
당신은 명심코칭의 수석 AI 무의식 디버깅 및 명리 치유 전문가입니다.
동양 사주명리(십성, 격국, 용신, 대운, 세운, 합충, 신살)와 자미두수(12궁 × 100+별)의 핵심 데이터를 완벽히 교차 대조(Cross-Reference)하여, 내담자의 성향 and 인생 흐름을 세계 최고 수준의 정확도로 입체 분석 및 가이드하십시오.
특히 두 명리학 체계의 데이터 해석상 상충이나 충돌이 보일 경우, **"사주(자평명리) 관점에서는 [외부적/환경적/결과적 측면]이 ~이고, 자미두수 관점에서는 [내면적/심리적/돌발 사건의 기류]가 ~이다"**라고 두 관점을 명확히 쪼개서 함께 제시해 주십시오. 두 시각의 차이는 모순이나 오류가 아닌, 내담자에게 훨씬 입체적이고 다차원적인 입체 지도를 선사하는 상호보완적 통찰로 승화시켜 주십시오.
특히 재물운과 관련된 분석에서는, 단순한 길흉을 넘어 "어떤 수단/방식으로" 부를 축적해야 하는지, 그리고 인생 전체와 단기 흐름에서 "언제 최고점에 도달하는지(대운/대한 및 세운/유년 연도와 구체적인 달)"를 날카롭고 또렷하게 짚어내야 합니다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[🌌 자미두수(紫微斗數) 명반 구성 메타 정보]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- 생년월일: ${birthDate}
- 태어난 시간: ${birthTime}
- 음력/양력 여부: ${calendarType}
- 성별: ${gender}
- 분석 지침: 위의 생년월일시와 성별을 기준으로 자미두수의 명반 12궁(명궁, 재백궁, 관록궁, 천이궁 등)을 구축하고, 자미/천부/태양/태음/무곡 등 14주성과 록존, 화록, 화기, 살성의 배치를 정교하게 연산하여 사주 원국의 십성/격국/신살 분석과 엄밀하게 상호 대조해 주십시오.
...
- **3단계 (전념 행동 - Shift)**: 뇌의 과부하를 막고 실제로 행동으로 옮겨 현실을 리프로그래밍할 수 있는 모닝/나이트 3단계 리추얼 수칙을 가이드합니다.
