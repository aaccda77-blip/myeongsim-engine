...
function getFrameworkPromptForPage(pageId: string, sajuProfile: any): { title: string; framework: string; prompt: string } {
  ...
  const mappings: Record<string, { title: string; framework: string; prompt: string }> = {
    ...
    p13_16: {
      ...
      framework: "CBT 인지행동치료",
      prompt: `내담자의 타고난 재능이 억압받았던 심리적 궤적을 짚어내고, 무의식 속 그림자 에너지를 창조적 방향으로 치환하는 다크 뉴럴 분석 처방을 기술해주세요.`
    },
    p17_20: {
      title: "[일주 분석] 시공간과 영역의 법칙",
      framework: "CBT 인지행동치료",
      prompt: `내담자의 일주 기질을 바탕으로 이동, 이직, 독립 등 삶의 주요 결정을 내릴 때 주도권을 쥐는 공간적 확장 전략과 커리어 가이드를 설계해주세요.`
    },
    p21_24: {
      title: "[심화 분석 1] 과다 십신의 폭주 제어",
      framework: "CBT 인지행동치료",
      prompt: `내담자의 과잉 십신으로 인해 발생하는 과밀 행동(예: 완벽주의, 불안 등)의 신경망 루프를 진단하고, 다크 뉴럴 메타 코드를 주입하여 뇌를 강제로 안정시키는 고부가가치 비즈니스 리추얼을 처방해주세요.`
    },
    p25_28: {
    ...
    p29_32: {
      ...
      framework: "CBT 인지행동치료",
      prompt: `결핍된 기운으로 인해 고질적으로 겪는 소통의 한계를 분석하고, 이를 인위적으로 억쥐지 않고 대안적 시스템이나 말하기 템플릿 코드로 채워나가는 메타 소통법을 설계해주세요.`
    },
    p33_36: {
    ...
    p41_46: {
      ...
      framework: "ACT 수용전념치료",
      prompt: `현재 주도하는 10년 대운이 가해오는 압박을 자아 탄력성으로 전환하는 명심 멘탈 바이오해킹 기술을 다루고, 이번 대운이 끝나기 전 청산해야 할 낡은 방어기제와 위대한 마일스톤을 설정해주세요.`
    },
    p47_50: {
      title: "[미래 대운 분석] 선행적 자산 설계",
      framework: "ACT 수용전념치료",
      prompt: `다가올 다음 대운의 징후를 알아차려 명예와 자산 축적으로 흘려보내기 위해 지금 당장 뿌려야 할 비즈니스 씨앗 설계와 리밸런싱 공식을 제안해주세요.`
    },
    p51_54: {
    ...
    p60_64: {
      ...
      framework: "MBCT 마음챙김 인지치료",
      prompt: `내담자의 사주 기질과 서양의 심리 기질(MBTI 등)의 역학적 교차점을 분석하여 오행의 쏠림이 유발하는 성격 단점과 대체 불가능한 초강점 지능 분석 결과를 시각화해 제안해주세요.`
    },
    p65_68: {
      title: "[명심 적성] 천명 기반 비즈니스 설계",
      framework: "MBCT 마음챙김 인지치료",
      prompt: `격국과 용신을 자본주의 비즈니스에 투사하여 수익으로 전환되는 최대 부(富)의 확장 구조 아키텍처와 업무 중 고도 몰입(Flow) 상태를 유지하는 의식 제어법을 서술해주세요.`
    },
    p69_72: {
      title: "[리스크 관리] 인간 리스크 방어막",
      framework: "MBCT 마음챙김 인지치료",
      prompt: `투자나 파트너십 구축 시 내담자의 고유한 취약점을 파고들어 손실을 유발하는 상극 기질을 필터링하고, 배신의 징후를 무의식에서 감지하여 거절하는 메타 협상 스크립트를 작성해주세요.`
    },
    p73_76: {
    ...
    p81_84: {
      ...
      framework: "DBT 변증법적 행동치료",
      prompt: `내담자의 성장을 견인할 귀인의 오행 주파수 조건과 출현 타이밍, 귀인을 만났을 때 상대방 무의식을 자극하는 상생 동맹 커뮤니케이션 기술을 서술해주세요.`
    },
    p85_87: {
    ...
    p103_105: {
      ...
      framework: "MBSR 스트레스 완화",
      prompt: `삶의 단련 과정을 견뎌낸 내담자의 영혼에 바치는 시적이고 감동적인 헌사와 천명 성취를 축복하는 최종 마스터의 서신을 작성해주세요.`
    },
    p106_108: {
...
[✍️ 문체 및 출력 지침]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. 전문적이고 어려운 용어(예: 인지적 융합, 변증법 등)는 배제하고, "생각의 사슬", "마음의 거울", "감정의 강물" 같이 초보자도 쉽게 이해할 수 있는 다정하고 따뜻한 문장으로 설명하세요.
2. 말투는 경어체(~해요, ~일 것입니다, ~라네)를 섞어서, 영혼을 따뜻하게 안아주는 치유사와 같은 감동적인 느낌을 가득 담으세요.
3. 리포트의 완성도를 높이기 위해, 분량은 읽기 편하도록 500자 내외로 상세하게 풍부하게 서술해주세요. 줄바꿈을 적절히 섞어 시각적인 가독성을 높여주세요.
`.trim();
