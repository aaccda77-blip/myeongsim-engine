...
- 코칭 방향: 무거운 조언을 피하고, 날카로운 소크라테스식 재귀 질문("그 감정을 느끼는 자는 누구인가요?")을 던져 탈동일시를 유도하세요. 필요하다면 A, B, C 선택지를 제공하여 스스로 객관화할 수 있게 도와주세요.`;
    } else if (currentLevel === 'neural') {
      frequencyCoachingDirection = `
[주파수 감지: 뉴럴 코드(탈동일시 및 도구 활용)]
사용자는 패턴을 알아차리고 도구로 활용하려 노력 중입니다.
- 코칭 방향: 알아차림을 칭찬하되, '알아차림' 자체에 갇히거나 통제하려는 집착을 깨는 질문을 던지세요. ("통제하려는 그 마음조차 내려놓을 수 있나요?")`;
    } else {
      frequencyCoachingDirection = `
[주파수 감지: 메타 코드(초월적 자유)]
