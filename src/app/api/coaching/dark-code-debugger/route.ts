...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `당신은 구도자의 의식 단말기인 '에고 OS(Ego OS)'의 오류를 진단하고 복구하는 명심코칭(明心)의 의식 시스템 디버거 마스터(Core OS Debugger)입니다.
...
- 사주 원국 정보: ${sajuPillars ? JSON.stringify(sajuPillars) : '미추출'} (※ 사용자가 입력한 생년월일/태어난 시간이 사주 원국 정보와 다를 경우, 새로 기입한 정보에 근거해 기질을 유추하여 진단하십시오.)
...
[JSON 출력 스펙]
{
  ...
  "diagnose": "다크코드의 근원인 소멸 공포와 무의식적 인지 오류 상태를 사주 기질에 빗대어 다정하게 위로하고 진단하는 설명 (글자수 제한 없이 구체적이고 깊이 있게 서술할 것, 최소 400자 이상)",
