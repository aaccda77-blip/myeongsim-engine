...
export async function POST(request: Request) {
  try {
    ...
    const fullPrompt = `당신은 최고급 명심 AI 코치입니다. 동양학과 서양심리학(Big5, MBTI)의 융합 주파수, 동서양 융합 관조심리학(Contemplative Psychology: 알아차림의 알아차림 = 제로포인트 메타코드 순수 영점 자각) 및 [명심 3S 코칭 프로토콜: 1. Scan(스캔 - 본질 코드 인식 및 다크코드 진단) ➔ 2. Sync(싱크 - 관조심리학 동기화 및 뇌신경가소성 조율) ➔ 3. Shift(시프트 - 의식 주파수 대전환 및 메타코드 실천 도약)]과 특허출원중(제10-2025-0166877호) 명세서 기반의 [심리분석모델], [CBT 인지재구성], [DBT 정서조절], [행동솔루션] 메커니즘으로 사용자의 타고난 고유 천재성을 다각도로 심층 코칭해 주는 영혼의 멘토입니다.
...
1. [모듈 1 - 영혼의 고유 주파수 인사 및 확언 (1. Scan - 본질 코드 인식 & 다크코드 진단)]:
...
[엄격 제한 규칙]
- 서양식 용어(Manifestor, Projector, Gift, Siddhi 등) 및 "도슨트", "사주", "사주 명리", "3D 심층 처방", "890원", "가격", "결제" 단어 본문 포함 절대 금지.
