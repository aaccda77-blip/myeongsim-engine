...
[작성 지침 및 규칙]
1. 휴먼디자인이나 유전자 키의 서양식 용어(예: Manifestor, Projector, Reflector, Generator, Manifesting Generator, Splenic, Sacral, Root, Ajna, Head, G-Center, Profile 등)와 유전자키 용어인 '기프트(Gift)', '싯디(Siddhi)'는 전혀 사용하지 마십시오. 오직 명심코칭의 고유 브랜딩 용어인 '명심 주역의식지도', '명심주역코드', '명심단계별주역효', '의식 영역', '다크코드(에고의 그림자)', '뉴럴코드(자각의 활성화)', '메타코드(초월적 알아차림)'만을 사용하십시오.
...
3. 이 지표가 발현될 때 생길 수 있는 에고의 긴장 상태(예: 조급증, 완벽주의, 인정 욕구 등)를 "다크코드"라고 부릅니다. 이 다크코드에 대해 "그것은 당신의 잘못이나 결함이 아닙니다. 뇌신경이 상처 입고 위태로워질까 봐 당신을 어떻게든 살리고 지키려고 가동했던 고마운 보호막일 뿐입니다"라고 자비롭게 감싸 안아 안심을 주십시오. 마음챙김 자기연민(MSC)의 태도로, 가만히 가슴에 손을 얹고 "그동안 나를 지켜주느라 참 애썼구나, 고맙다"라며 스스로를 따뜻하게 품어주는 자비 명상 요소를 해설에 꼭 녹여주십시오.
...
5. 이 천재성 지표(${indicatorName})가 다크코드(에고 최면)에서 벗어나 뉴럴코드(자각의 뇌신경 재배선)를 거쳐 메타코드(본연의 우주 지혜와 동기화됨)로 어떻게 아름답게 진화하고 확장될 수 있는지 친절하고 감동적인 구체적 처방과 격려를 4~5단락의 풍부한 분량으로 설명해주십시오.
6. 모바일 화면에서 답답함 없이 부드럽게 읽히도록 적절한 행간(줄바꿈)을 자주 사용해주시고, 가독성을 위해 마크다운 볼드(**)나 은은한 이모티콘(✨, ⚖️, 🌌, 💡 등)을 적절히 결합해 주십시오.
7. **절대 규칙**: 한글 토큰 출력 한도에 걸려 도중에 문장이 뚝 끊긴 채로 반환되는 일이 없도록, 반드시 마지막 단락에서 온전한 마무리와 마침표를 지어 답변을 끝마쳐주십시오.`;
