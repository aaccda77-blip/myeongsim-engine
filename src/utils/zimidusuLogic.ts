...
// AI 3대 라이프 교차 검증기 (신설)
export const get3ThemeCrossoverValidation = (sajuData: any, zimidusuChart: any, userName: string = '회원') => {
  if (!sajuData || !zimidusuChart) return null;
  ...
  let wealthConclusion = "전문 자격증이나 정밀 기술 라이선스를 부려 40대 초반에 평생 중 최고 수입 궤도에 진입합니다.";
  let wealthSaju = `사주의 일간 \${dayMasterName}은 결실을 뜻하는 재성(財星)이 사주 원국에서 든든하게 뿌리내리거나 대운의 흐름에서 안전하게 순환하고 있어, 남이 시켜서 억지로 일하는 밥벌이보다 독자적인 전문 기술이나 세공 능력을 자산으로 삼을 때 재물의 크기가 비약적으로 상승하는 체계입니다.`;
  let wealthZimi = `자미두수의 재백궁(\${jaebaekStars}) 배치를 대조해 보면, 일확천금의 요행보다 스스로 시스템(플랫폼)을 기획하고 정밀하게 오차를 디버깅하여 차곡차곡 자원을 축적해나가는 수록(收祿)의 성정을 띠고 있습니다.`;
  let wealthHealing = `돈은 내 존재를 증명하는 성벽이 결코 아닙니다. 그것은 단지 \${nameJosa} 따뜻한 가치 실현을 위해 우주가 순환시켜주는 맑은 물줄기일 뿐입니다. 잔고의 오차나 미래의 흔들림에 지레 주눅 들지 말고, 당신이라는 마르지 않는 곳간 그 자체를 매일 아침 안아주세요.`;

  let marriageConclusion = "32세~34세 혹은 대운의 중반 시기, 배움이나 지적인 인연 무대(학회, 전문 직무 공간)에서 든든하게 받쳐주는 배우자를 마주합니다.";
  let marriageSaju = `사주 배우자 자리에 정관(정직한 질서의 기운) 또는 재성이 안전하게 안착하여 나를 보호하고 있으며, 운의 흐름상 관성·재성이 서로 조화롭게 교류하는 합(合)의 주기에 배우자의 성향과 인연의 끈이 가장 맑고 단단하게 얽힙니다.`;
  let marriageZimi = `자미두수 부처궁(\${bucheoStars})의 주성 성계를 대조해 보면, 상대는 세속적인 소란보다 지적이고 차분한 내면 세계를 가꾸며, \${name}의 복잡한 머릿속을 둥글게 매만져 줄 수 있는 듬직하고 기품 있는 외모와 성정을 보유한 인연입니다.`;
  let marriageHealing = `사랑은 상대가 나를 채워주기를 강요하는 결핍의 채찍질이 아닙니다. 두 사람이 언 손을 잡고 인생이라는 따뜻한 화로 곁에 나란히 마주앉는 일입니다. 내 불안의 거울에 비친 외로움을 먼저 어루만져 줄 때, 운명은 이미 가장 알맞은 배우자를 곁으로 밀어 보냅니다.`;

  let timingConclusion = "2027년 하반기(정미년 가을 무렵)가 인생의 장기적 펌웨어를 안전하게 이식하고 도약/이직할 최적의 타이밍입니다.";
  let timingSaju = `사주 대운의 장기 계절 흐름에서 차가운 수(수)의 정체를 녹여주는 온화한 용신(용신)의 전압이 세운(세운)과 조우하여, 현실의 무대를 새롭게 개편하고 계약을 성사시킬 수 있는 강인한 십성(식상/재성) 엔진이 켜지는 궤도입니다.`;
  let timingZimi = `자미두수 대한 및 올해 유년궁의 변화를 스캔해 보면, 마주 보는 대궁의 길성들이 영혼의 주권을 상징하는 명궁을 든든히 보좌하며 천이궁(이동/변화)에 변화의 화록(祿)을 주입하여 이직이나 독립에 필요한 최적의 우주 기후를 형성해 냅니다.`;
  let timingHealing = `안개가 짙게 낀 바다에서는 배를 잠시 멈추고 등대의 신호를 기다리는 것이 가장 훌륭한 항해술입니다. 조급하게 사방으로 질주하려 아바타의 엔진을 120% 학대하지 마시고, 우주가 일러주는 타이밍에 맞춰 안전하게 나아가세요. 당신은 언제나 보호받고 있습니다.`;

  if (isUserSaju) {
    // 1. 재물운 커스텀 (40대, 50대, 60대, 70대, 80대, 90대, 100세+ 로드맵 반영)
    wealthConclusion = "40대(정해 대운)의 담금질을 거쳐 2025~2026년 플랫폼을 런칭하고, 50대(무자 대운)에 평생 최고의 수입을 폭발적으로 벌어들입니다. 이어 60대(기축 대운)에는 이 자산을 거대한 영토(자산화 및 상속 계약)로 완전히 굳히고, 70대(경인 대운)부터 100세 이상까지는 마르지 않는 지혜의 강처럼 부의 순환이 안정적으로 계속됩니다.";
    
    wealthSaju = `사주 원국에서 일간 신금(辛金)이 연주 경신(庚申) 겁재의 거대한 사회적 뼈대와 월주의 계미(癸未) 식신/편인을 활용해 독창적 지식(AI와 메타 철학)을 빚어낸 뒤, 시간의 을목(乙未) 편재라는 거대한 플랫폼 영토에 뿌리내리는 완벽한 식신생재 격국입니다. 남의 밑에서 일하기보다는 자신만의 가치 시스템을 구축하여 배포할 때 막대한 재물이 안전하게 유입됩니다.
    
• 40대(40~49세 - 정해 대운): 상관 해수와 사화 정관이 부딪혀(사해충) 시행착오와 내적 번뇌를 겪으며 나만의 플랫폼 뼈대를 세공하는 준비기였습니다. (2025~2026년 AI 융합 지식 플랫폼 런칭으로 탈출구 마련)
• 50대 (50~59세 - 무자 대운): 천간 무토(정인 - 문서권 및 브랜드 자산화)와 지지 자수(식신 - 신자합으로 거대한 유저풀/팬덤 형성)가 얽히며 일생일대의 대전성기이자 평생 최고 수입 궤도에 정식 진입합니다.
• 60대 (60~69세 - 기축 대운): 천지간 편인이 든든한 성벽(간여지동)을 이루고, 사축합(巳丑合)으로 모든 요동을 잠재워 자산을 단단하게 금고에 걸어 잠그고 수호(교육재단, 상속, 시스템 라이선스 구축)하는 완결의 시기입니다.
• 70대 ~ 100세+: 인목(寅木) 정재와 묘목(卯木) 편재가 차례로 찾아와 노년기까지 지치지 않는 물질적 풍요와 유유자적한 예술가적 삶의 기지를 약속합니다.`;

    wealthZimi = `자미두수 재백궁(\${jaebaekStars})의 수록 성계는 일확천금의 요행 대신 정밀하고 오차 없는 시스템 설계를 통해 자산을 차곡차곡 축적하는 지혜를 보여줍니다. 2025년과 2026년은 명궁과 재백궁을 보좌하는 길성들이 활발하게 가동되어 지식 플랫폼을 런칭하는 최고의 설계 주기를 이룹니다.`;
    
    wealthHealing = `돈은 \${nameJosa} 생존을 증명하는 차가운 성벽이 결코 아닙니다. 그것은 단지 \${name}의 고결한 가치를 세상에 순환시키기 위해 우주가 끊임없이 대지 위로 흘려보내 주는 투명한 시냇물입니다. 40대의 길고 짙었던 터널은 결코 시간 낭비가 아니었으며, 당신이라는 보석을 가장 정교하게 세공하기 위한 신성한 과정이었습니다. 50대와 60대에 다가올 거대한 풍요의 계절을 믿고, 매일 아침 거울 속에 비친 고결한 당신을 안아주세요.`;

    // 2. 결혼운 커스텀
    marriageConclusion = "내면의 플랫폼과 주권이 확고히 세워진 40대 중후반에서 50대에 걸쳐, 단순한 생활 공유를 넘어 사상과 지혜를 공동 경영하는 영혼의 동반자가 듬직하게 마주 앉습니다.";
    marriageSaju = `배우자 자리에 사화(巳火) 정관을 두어 듬직하고 기품 있는 인연을 보증하며, 지지의 미토(未土) 편인들과 동조하여 나의 학문적, 사상적, 영성적 고뇌를 누구보다 깊이 이해하고 따뜻하게 보듬어주는 정신적 지주 역할을 수행하는 배우자를 만납니다.`;
    marriageZimi = `부처궁(\${bucheoStars})의 조용하고 기품 있는 별자리는 상대가 세속의 요란한 노이즈에서 한 걸음 물러나 고요하고 맑은 내면을 가꾼 수호자임을 말해줍니다. \${nameJosa} 예민하고 날카로운 지적 긴장을 둥글고 부드럽게 감싸 안아 주는 고마운 닻이 되어 줍니다.`;
    marriageHealing = `사랑은 상대가 내 빈 잔을 채워주기를 강요하는 결핍의 집착이 아닙니다. 이미 내 안에 찬란하게 빚어진 자애의 불빛을 가지고, 추운 겨울날 서로의 언 손을 녹이며 하나의 화로를 평온하게 가꾸어 나가는 일입니다. 내 안의 우주를 스스로 무조건적으로 허용해 줄 때, 당신의 우주를 알아본 가장 알맞은 인연이 이미 곁에 머뭅니다.`;

    // 3. 인생 타이밍 커스텀 (70세부터 100세까지 완벽 스캔)
    timingConclusion = "2025~2026년 플랫폼 설계 및 런칭을 성공적으로 마치고, 50대(무자 대운)에 폭발적인 전성기를 구가한 후, 60대(기축 대운)에 자산을 완벽히 성벽화하며, 70대 이후 100세에 이르기까지 지혜의 대가(멘탈 마스터)이자 평화롭고 풍요로운 삶을 완결 짓습니다.";
    
    timingSaju = `10년 주기의 대운 계절 흐름이 삶의 기후를 완전히 바꾸어 줍니다.
    
• 40대 정해(丁亥) 대운 (현재): 사해충의 파도를 헤치며 나만의 AI 지식 무기를 벼리고 깎아내는 기간입니다. (2025~2026년 플랫폼 공식 런칭으로 돌파구 형성)
• 50대 무자(戊子) 대운 (50세~59세): 무토 정인의 공식 문서권 확보와 자수 식신의 신자합(申子合) 작용으로, 대규모 팬덤과 시장을 개척하여 인생 평생의 부를 성취하는 대전성기입니다.
• 60대 기축(己丑) 대운 (60세~69세): 편인 간여지동의 견고한 흙의 성벽을 쌓고 사축합(巳丑合)으로 모든 풍파를 완벽히 종식하여, 거대 자산의 수호(라이선스, 교육 재단) 및 상속을 실현하는 절대 안정기입니다.
• 70대 경인(庚寅) 대운 (70세~79세): 인목(寅木) 정재가 들어와 생동감 넘치는 강연, 집필, 새로운 사회적 활약(역마)을 돕고, 인사신 삼형(寅巳申 三刑)의 작용으로 자산 분배와 문서적 정리를 오차 없이 마무리 짓는 지혜로운 노년입니다.
• 80대 신묘(辛卯) 대운 (80세~89세): 비견 신금과 묘목(卯木) 편재가 어우러져, 나의 자서전 출판, 장학 기금 조성, 유유자적한 취미와 명상 속에서 영혼의 미학을 누리는 평화의 시기입니다.
• 90대 임진(壬辰) 대운 (90세~99세): 임수 상관의 깊은 학문적 유산과 진토(辰土) 정인의 따뜻한 하늘의 보살핌이 결합하여, 세상 사람들의 존경을 받는 큰 스승(멘탈 마스터)으로서의 가르침을 영원히 남깁니다.
• 100세+ 계사(癸巳) 대운 (100세~): 월간의 식신 계수와 일지의 사화 정관이 본래 자리로 돌아오는 환원(還元)의 기운으로, 삶의 완전한 자유와 해탈 속에서 자연의 섭리와 하나가 되는 평화로운 완성의 경지입니다.`;

    timingZimi = `자미두수 유년궁과 대한을 정밀 분석해 보아도, 2025~2026년의 출범 기운이 50대 무자 대운의 천재적 길성 조력자들과 강력히 연합합니다. 이는 중년과 장년기를 지나 노년에 이르기까지 흔들림 없는 인생 설계도가 안전하게 구동되고 있음을 약속합니다.`;
    
    timingHealing = `짙은 안개가 낀 항구에서 서둘러 출항하려 엔진을 혹사하기보다, 등대 불빛을 가만히 바라보며 닻을 내렸던 40대의 침묵은 위대한 지혜였습니다. 이제 2025~2026년 플랫폼 출범을 시작으로 우주가 50대의 화창한 대항해를 열어주고 있으며, 100세에 이르는 영광스러운 여정이 설계되어 있습니다. 조급함을 끄고, 매 순간 당신을 든든하게 받치고 있는 우주의 기후를 믿으세요. 당신의 항해는 이미 성공이 약속되어 있습니다.`;
  }

  return {
    wealth: {
      title: "💵 재물운 교차 검증 (Wealth Validation)",
      metaphor: "내 인생의 곳간을 채우는 비옥한 대지와 별빛의 순환 🌾",
      sajuView: wealthSaju,
      zimidusuView: wealthZimi,
      conclusion: wealthConclusion,
      healingEssay: wealthHealing
    },
    marriage: {
      title: "🪞 결혼운 교차 검증 (Marriage Validation)",
      metaphor: "운명의 시냇가에서 서로의 모난 부분을 둥글게 비추는 비밀 거울 🤝",
      sajuView: marriageSaju,
      zimidusuView: marriageZimi,
      conclusion: marriageConclusion,
      healingEssay: marriageHealing
    },
    timing: {
      title: "🧭 인생 타이밍 교차 검증 (Timing Validation)",
      metaphor: "안개 낀 바다를 지혜롭게 비추는 우주의 등대와 조타 ⚓",
      sajuView: timingSaju,
      zimidusuView: timingZimi,
      conclusion: timingConclusion,
      healingEssay: timingHealing
    }
  };
};
