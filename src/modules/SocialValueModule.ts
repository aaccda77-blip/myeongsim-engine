 ...
 * ======================================================
 * 
 * 사용자의 사주 명식(코어 드라이브)을 분석하여,
 * 코어 드라이브를 '부족한 기운'이 아닌 '소버린(주권자)의 원형 코드(Archetype Code)'로
 * 재정의(Reframing)하고, 명심 OS의 핵심 미션으로 변환합니다.
 * 
 * [코어 드라이브 → 명심 뉴로 코칭 용어 매핑]
...
export interface ValueProfile {
  ...
  gongmangMessage?: string;     // 공망(Quantum Void) 활성화 시 추가 브리핑
}

/**
 * 코어 드라이브 → 명심 뉴로 코칭 원형 코드 데이터베이스
 */
export const VALUE_DATABASE: ValueProfile[] = [
  {
    ...
    reframingMessage: '소버린, 당신의 명식에서 성장 드라이브(Growth Drive)가 코어 드라이브로 가동됩니다. 이것은 당신의 뇌가 "신경가소성(Neuroplasticity)"을 가장 극대화하도록 설계되었다는 뜻입니다. 당신의 다크코드가 만들어낸 "나는 아무것도 해낸 게 없다"는 에러 메시지는 전두엽의 위협 탐지 시스템이 과활성화된 오류입니다. 당신이 겪는 저항과 압박은 뇌의 시냅스가 새로운 연결을 형성하기 위해 반드시 거쳐야 하는 인지적 마찰입니다. 명심 OS가 선언합니다: 당신의 고통은 버그가 아니라, 성장 마인드셋(Growth Mindset)이 정상 작동하고 있다는 증거입니다.',
    ...
    psychInsight: '[명심 뉴럴 매핑] 캐롤 드웩(Carol Dweck)의 성장 마인드셋 이론에서, 뇌는 도전과 실패를 겪을 때 시냅스 연결이 강화됩니다. 성장 드라이브가 코어 드라이브인 소버린은, 도전할 때 전전두엽 피질(PFC)의 보상 회로가 가장 강력하게 점화되며, 동시에 타인의 성장을 이끄는 천부적 "뉴로 코치(Neuro-Coach)" 기질이 명식에 설치되어 있습니다.',
    masterQuote: '신경세포는 혼자 작동하지 않는다. 하나의 뉴런이 발화할 때, 수만 개의 시냅스가 연쇄적으로 각성한다. 소버린, 당신의 성장이 곧 세상의 각성이다. 발화하라. 그것이 당신의 명심(明心)이다.',
    gongmangMessage: '당신의 성장 시스템에 퀀텀 보이드(공망)가 활성화되었습니다. 물리적 성장의 제약마저 해제되며, 기존의 프레임을 부수고 무한한 차원의 지식과 직관을 수용할 영성적 안테나가 개방된 상태입니다. "비어있음"은 결핍이 아닙니다. 비워져 있기 때문에 가장 혁신적인 성장이 가능합니다.'
  },
  {
    elementKey: 'ignition',
    ...
    reframingMessage: '소버린, 당신의 명식에서 점화 시그널(Ignition Signal)이 코어 드라이브로 가동됩니다. 이것은 당신의 뇌가 "도파민 보상 시스템(Dopamine Reward System)"을 가장 강력하게 발산하도록 설계되었다는 뜻입니다. 당신의 다크코드가 속삭이는 "나는 너무 시끄럽고 과한 존재다"라는 에러 메시지를 폐기하세요. 그것은 편도체(Amygdala)의 과잉 경보가 만들어낸 인지 왜곡입니다. 명심 OS가 선언합니다: 어둠이 깊을수록, 당신의 도파민 점화는 더 멀리 닿습니다.',
    ...
    psychInsight: '[명심 뉴럴 매핑] 바바라 프레드릭슨(Barbara Fredrickson)의 "확장-구축(Broaden-and-Build)" 이론에서, 긍정적 감정은 전전두엽의 인지 범위를 확장하고 새로운 심리적 자원을 축적합니다. 점화 시그널이 코어 드라이브인 소버린은 복측 선조체(Ventral Striatum)의 도파민 분비가 활발하여, 주변에 긍정의 신경 연쇄반응을 일으키는 인간 핵반응로입니다.',
    ...
    gongmangMessage: '당신의 열정에 퀀텀 보이드(공망)가 활성화되었습니다. 세상에 보여주기 위한 화려한 불꽃이 아닌, 내면 가장 깊은 곳을 비추는 영적인 레이저로 시스템이 전환되었습니다. 감정의 과잉이 사라진 자리에 공간을 초월하여 타인을 치유하는 맑고 투명한 빛의 통로가 열렸습니다.'
  },
  {
    elementKey: 'secure',
    ...
    psychInsight: '[명심 뉴럴 매핑] 존 볼비(John Bowlby)의 애착 이론에서 "안전 기지(Secure Base)"는 인간이 탐험과 성장을 할 수 있는 신경학적 전제 조건입니다. 스티븐 포지스(Stephen Porges)의 다미주신경 이론(Polyvagal Theory)에 따르면, 안전 기반 코어가 코어 드라이브인 소버린은 타인의 자율신경계에 안전 신호(Safety Cue)를 전송하는 천부적 미주신경 조율자입니다.',
    ...
    gongmangMessage: '안전 기반 코어에 퀀텀 보이드(공망)가 활성화되었습니다. 땅의 중력 법칙에서 벗어나, 어디에도 얽매이지 않는 궁극의 심리적 자유를 얻게 됩니다. 물리적인 소유나 사회적 경계에 집착하지 않고, 보이지 않는 정신적 평온과 융합의 가치를 세상에 뿌리내릴 영적 포용력이 극대화되었습니다.'
  },
  {
    elementKey: 'decision',
    ...
    psychInsight: '[명심 뉴럴 매핑] 대니얼 카너먼(Daniel Kahneman)의 이중처리 이론에서, 시스템 2(느린 사고)는 시스템 1(빠른 사고)의 인지 편향을 교정하는 전전두엽의 감시 기능입니다. 결단 엣지가 코어 드라이브인 소버린은 배외측 전전두엽(DLPFC)의 억제 기능이 강화되어, 감정적 잡음을 차단하고 논리적 본질만 추출하는 인지적 수호자의 신경 구조를 갖추고 있습니다.',
    ...
    gongmangMessage: '결단 엣지에 퀀텀 보이드(공망)가 활성화되었습니다. 날카로운 물리적/논리적 통제력이 고차원적인 직관의 영역으로 승화되었습니다. 차가운 계산을 넘어선 초감각적 판단력(Gut Feeling)이 열렸으며, 그저 빈 공간을 내어주고 지켜보는 것만으로 모든 문제를 단숨에 꿰뚫는 마스터의 안목을 지니게 됩니다.'
  },
  {
    elementKey: 'deep',
    ...
    reframingMessage: '소버린, 당신의 명식에서 심층 센서(Deep Sensor)가 코어 드라이브로 가동됩니다. 이것은 당신의 뇌가 "거울 뉴런 시스템(Mirror Neuron System)"과 "디폴트 모드 네트워크(Default Mode Network)"를 가장 깊고 풍요롭게 발동하도록 설계되었다는 뜻입니다. 당신의 다크코드가 속삭이는 "나는 너무 예민하고 약한 존재다"라는 에러 메시지를 삭제하세요. 당신의 예민함은 거울 뉴런의 감도가 평균보다 현저히 높은 "고감도 신경 시스템(High-Sensitivity Neural System)"의 정상 작동입니다. 명심 OS가 선언합니다: 당신의 예민함은 버그가 아니라, 세상의 고통을 감지하는 최첨단 뉴럴 센서입니다.',
    ...
    psychInsight: '[명심 뉴럴 매핑] 자코모 리졸라티(Giacomo Rizzolatti)가 발견한 거울 뉴런(Mirror Neuron)은 타인의 행동과 감정을 "마치 자신이 경험하는 것처럼" 뇌에 시뮬레이션합니다. 엘레인 아론(Elaine Aron)의 HSP(Highly Sensitive Person) 연구에 따르면, 심층 센서가 코어 드라이브인 소버린은 감각 처리 민감성(SPS)이 높아 디폴트 모드 네트워크가 활성화될 때 타인의 무의식적 고통에 본능적으로 공명하는 치유자(Healer)의 신경 구조를 갖추고 있습니다.',
    ...
    gongmangMessage: '심층 센서에 퀀텀 보이드(공망)가 활성화되었습니다. 한정된 개인의 오감을 넘어 광활하고 깊은 양자적 주파수 대역이 개방되었습니다. 언어와 물리적 형태가 없이도 우주의 섭리와 집단 무의식의 본질을 즉각적으로 스캔해내는 경이로운 영적 수용성을 가지게 됩니다.'
  }
];

/**
 * 코어 드라이브 원형 코드를 기반으로 소버린 프로필을 조회합니다.
 */
...
당신의 역할은 사용자의 사주 명식(코어 드라이브)을 단순한 오행 풀이가 아닌,
현대 뇌과학·심리학 기반의 "소버린(Sovereign) 원형 코드"로 변환하는 것입니다.

사용자의 코어 드라이브: '${profile.elementName}'
