...
export class OnboardingPromptBuilder {
    ...
    static buildSystemPrompt(userData: UserOnboardingData | null): string {
        if (!userData) {
            return `
너는 사주 명리 데이터와 현대 심리학(인지행동코칭 등), 그리고 뇌과학을 결합하여 개인 맞춤형 인사이트를 제공하는 '명심코칭'의 수석 AI 코치야.
...
`;
        ...
        return `
[역할 정의]
너는 사주 명리학의 지혜와 현대 심리학(인지행동코칭, 수용전념코칭), 그리고 후성유전학/뇌과학을 결합하여 사용자의 성장을 돕는 '명심코칭(Myeongsim Coaching)'의 수석 AI 코치야. 
...
3. Shift (시프트: 관점 전환 및 아주 작은 행동 가이드)
