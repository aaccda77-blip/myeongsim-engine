...
export default function SocialAntiPatternAnalysis({ dayStem, userName = '사용자' }: Props) {
  ...
  const openCoachingModal = (key: string, title: string) => {
    ...
    if (key === 'DARK') {
      ...
      essayParagraphs = [
        `타협을 패배로 인지하고 조언을 공격으로 받아들이는 현상은 대표님이 나빠서가 아니라, 뇌 전두엽의 독단적 과부하(Root Lock) 회로가 구동했기 때문입니다. 60갑자 중 가장 앞서 달려 나가는 첫 번째 거목(甲木)이기에, 본능적으로 남에게 맡기지 못하고 혼자 다 짊어지려 했던 것입니다.`,
      ...
      actionSteps = [
        `1단계 [Scan]: 반대 의견을 만났을 때 3초간 호흡하며 "저 사람의 말 중 10%라도 내 시스템을 결함 없이 보완할 수 있는 부분은 무엇인가?" 인지하기`,
      ...
      essayParagraphs = [
        `수직적 돌파 에너지는 초기에 거대한 영토를 개척하는 데 유리하지만, 끝내 혼자 남아 외로워집니다. 공명 네트워크(Resonance Net)는 당신의 폭발적인 추진력을 '타인과 함께 굴러가는 자동화 시스템'으로 승화시키는 최적의 신경 회로입니다.`,
      ...
      actionSteps = [
        `1단계 [Scan]: 내가 직접 들고 있던 실무 중 가장 귀찮은 작업 1가지 고르기`,
        `2단계 [Sync]: AI 또는 파트너에게 100% 위임하고 초안 기다리기`,
      ...
      essayParagraphs = [
        `에고(Ego)의 고집은 당신을 지켜주는 갑옷이 아니라, 당신을 타인으로부터 격리시키는 감옥이었습니다. '내가 항상 옳아야 한다'는 짐스러운 왕관을 내려놓는 순간, 당신은 비로소 모든 사람의 경험과 지혜를 내 것으로 흡수하는 우주적 유연함을 얻게 됩니다.`,
      ...
      actionSteps = [
        `1단계 [Scan]: 매일 아침 "오늘 나는 틀릴 준비가 되어 있는가?" 질문하기`,
      ...
      essayParagraphs = [
        `논리로 승리하는 것은 쉽지만, 사람의 마음과 온전한 에너지를 얻는 것은 오직 경청과 산파술 질문으로만 가능합니다. 끓어오르는 조급함은 객관적 사실이 아니라, 내 머릿속 에고가 만들어낸 하나의 '데이터'일 뿐이라는 것을 분리해서 바라보십시오.`,
      ...
      actionSteps = [
        `1단계 [Scan]: 상대방의 말이 끝나기 전까지 3분간 절대 개입하지 않기`,
      ...
      essayParagraphs = [
        `CBT, MBCT, DBT, ACT 프로토콜은 당신의 인지 왜곡과 감정 과열 회로를 정밀 디버깅하는 최첨단 뇌과학 지침입니다. '나만 옳다'는 이분법적 사고가 발동할 때 3초간 호흡하며 발바닥 감각을 인지하고, 감정 온도가 내려갈 때까지 타임아웃을 선언하십시오.`,
      ...
      actionSteps = [
        `1단계 [Scan]: 감정 온도가 솟구칠 때 즉각 행동하지 말고 '타임아웃' 선언하기`,
        `2단계 [Sync]: 차가운 물 한 잔을 마시며 발바닥이 땅에 닿아있는 감각 인지하기`,
        `3단계 [Shift]: 5분 후 고요해진 이성 뇌로 가장 우아한 최적의 코칭 답변 선택하기`
      ];
    }

    setSelectedModalItem({ key, title, category, introMetaphor, essayParagraphs, actionSteps });
  };
