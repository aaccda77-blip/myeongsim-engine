...
import { Lock, Sparkles, ArrowRight, ShieldCheck, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function SiteAccessGateOverlay() {
    ...
    useEffect(() => {
        setMounted(true);
        // Bypass for gate and admin login routes
        if (pathname === '/gate' || pathname === '/admin/login' || pathname?.startsWith('/api/gate')) {
            setIsUnlocked(true);
            return;
        }

        // Only check valid granted session
        const hasCookie = document.cookie.includes('myeongsim_site_access=granted') ||
    ...
    if (!mounted || isUnlocked || pathname === '/gate') {
        return null;
    }
    ...
    return (
        <div className="fixed inset-0 z-[999999] bg-slate-950 text-white flex flex-col items-center justify-center p-4 overflow-hidden font-sans selection:bg-amber-400 selection:text-slate-950">
