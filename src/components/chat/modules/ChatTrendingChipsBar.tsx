        ...
        <div className="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar py-0.5">
            <div className="flex items-center gap-1.5 shrink-0">
                {onOpenQ38Modal && (
                    <button
                        type="button"
                        onClick={onOpenQ38Modal}
                        className="text-[10px] sm:text-xs font-black px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-2xl bg-gradient-to-r from-emerald-600/90 via-teal-700/90 to-slate-900 hover:from-emerald-500 hover:to-teal-600 border border-emerald-400/80 text-emerald-100 hover:text-white transition-all whitespace-nowrap shrink-0 shadow-lg hover:shadow-[0_0_15px_rgba(16,185,129,0.5)] active:scale-95 cursor-pointer flex items-center gap-1 min-h-[36px]"
                    >
                        <span>💡 [Q38] ChatGPT vs 명심AI</span>
                    </button>
                )}
                {chips.map((chip, idx) => (
                    <button
