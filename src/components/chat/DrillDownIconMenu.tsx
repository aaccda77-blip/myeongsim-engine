...
export default function DrillDownIconMenu({
    ...
    return (
        <>
            ...
            {/* [NEW] Healing Music Player Modal - Enhanced with Playlist & Lyrics */}
            <AnimatePresence>
                {showMusicPlayer && (
                    <motion.div
                        initial={{ opacity: 0 }}
                    ...
                    >
                        ...
                        >
                            ...
                            {(() => {
                                ...
                                const [currentSong, setCurrentSong] = React.useState(HEALING_SONGS[0]);

                                return (
                                    <>
                                        {/* Song Selector */}
                                        <div className="flex flex-wrap gap-2 justify-center mb-4">
                                            {HEALING_SONGS.map((song) => (
                                                <button
                                                    key={song.id}
                                                    onClick={() => setCurrentSong(song)}
                                                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${currentSong.id === song.id
