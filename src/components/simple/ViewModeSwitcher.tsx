...
export function ViewModeSwitcher({ compact = false }: ViewModeSwitcherProps) {
    ...
    return (
        <div className="flex items-center p-0.5 sm:p-1 rounded-xl bg-[#0e1624] border border-white/10 shadow-inner shrink-0">
            {/* 1. 기본모드 Classic */}
            <button
                type="button"
                onClick={() => setViewMode('classic')}
                className={`flex items-center gap-1 px-1.5 py-1 sm:px-2.5 sm:py-1.5 rounded-lg text-[11px] sm:text-xs transition-all cursor-pointer whitespace-nowrap shrink-0 ${
                    viewMode === 'classic'
            ...
            >
                ...
                <span className={viewMode === 'classic' ? 'inline font-bold' : 'hidden sm:inline'}>기본</span>
            </button>
            ...
            >
                ...
                <span className={viewMode === 'refined' ? 'inline font-bold' : 'hidden sm:inline'}>리파인</span>
            </button>
            ...
            >
                ...
                <span className={viewMode === 'simple' ? 'inline font-bold' : 'hidden sm:inline'}>간편</span>
            </button>
            ...
            >
                ...
                <span className={viewMode === 'wearable' ? 'inline font-bold' : 'hidden sm:inline'}>워치</span>
            </button>
        </div>
    );
}
