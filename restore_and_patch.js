...
// 1. 기존 백업본에서 깨끗한 상태로 복원
let content = fs.readFileSync(backup, 'utf8');

// 2. 구문 오류 복구 (RELATION_DETAIL 끊긴 부분 복원)
...
// 3. 엉뚱하게 남아있던 이전 정적(Static) 코드 블록 완전히 삭제 (인덱스 기반으로 정확하게)
const floatStart = content.indexOf('// ─────────────────────────────────────────────\n, a: \'타인의 신경계와 내 신경계가 뒤섞여');
const floatStart2 = content.indexOf('// ─────────────────────────────────────────────\r\n, a: \'타인의 신경계와 내 신경계가 뒤섞여');
const startIdx = floatStart !== -1 ? floatStart : floatStart2;
const floatEnd = content.indexOf('// 메인 패널 컴포넌트 — 프리미엄 리디자인');

if (startIdx !== -1 && floatEnd !== -1) {
    content = content.substring(0, startIdx) + '// ─────────────────────────────────────────────\n' + content.substring(floatEnd);
}

// 4. 3S 패치 탭에 동적 렌더링 UI 업그레이드
...
content = content.replace(/\{activeTab === 'patch' && harmony && \([\s\S]*?<\/motion\.div>\r?\n\s*\}\)/m, patchTabNew);

// 5. DailyPatchSection 라디오 버튼 UI 스크린샷과 동일하게 스타일 업그레이드 (DailyPatchSection 내부만 치환)
...
const patchStart = content.indexOf('function DailyPatchSection(');
if (patchStart !== -1) {
  const patchSectionStr = content.substring(patchStart);
  const oldButtonRegex = /<button[\s]*?key=\{optIdx\}[\s\S]*?onClick=\{\(e\) => \{[\s\S]*?className=\{`w-full text-left p-2\.5 rounded-lg border text-\[11px\] transition-all \$\{selectedOptions\[idx\] === optIdx \? 'bg-white\/10' : 'border-white\/5 hover:bg-white\/5'\}`\}[\s\S]*?>\r?\n\s*\{opt\}\r?\n\s*<\/button>/;
  content = content.substring(0, patchStart) + patchSectionStr.replace(oldButtonRegex, buttonNew);
}

fs.writeFileSync(file, content, 'utf8');
console.log('✅ DailyBioSyncPanel.tsx 완벽 복구 및 동기화 완료!');
